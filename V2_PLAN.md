...
### Сессия B — шаг 4: вендоринг design_system

Механическая и объёмная: ~860 KB одним коммитом. Держать её отдельно от всего остального,
...
- **Ловушка:** `.gitignore` уже один раз молча съел файл шаблона (`.env.example.erb`).
  Перед коммитом проверить `git status --ignored` по новому дереву — там `node_modules`,
  `dist`, `*.css` могут попасть под шаблонные правила.
- **Готово, когда:** дерево лежит, `bin/sync_ds` печатает пустой диff, гейт не изменился
  (ничего ещё не подключено — Vite будет в сессии C).
...
|---|---|---|
| §4.2 — вендорим снапшот design_system (решение записано, ждёт возражений) | сессию B | ответа нет, действую по записанному |
