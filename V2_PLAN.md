...
### Сессия C — шаг 5 + решение §4.1: Vite, React, Tailwind

Самая рискованная. Здесь же **впервые полностью проверяется Docker-слой** — до этого сервис
`vite` в рестарт-цикле, потому что `bin/vite` не существует.

- `vite_rails` + `vite-plugin-ruby` + `@vitejs/plugin-react` по паттерну `~/works/cookware.me`.
- Два энтрипоинта: админка и TMA.
- `config/vite.json` — dev-сервер слушает `0.0.0.0`, Rails ходит на хост `vite`
  (в compose уже проставлены `VITE_RUBY_HOST`).
- **Решить §4.1 здесь же**, до того как таблицы расползутся по экранам.
- **Готово, когда:** `./run setup && ./run up` поднимает все три сервиса здоровыми и страница
  админки отдаёт собранный React. Это же закрывает «не проверено» из шага 2.
...
| §4.1 — DataTable клиентский или добавляем server-mode | сессию C, шаги 6–7 | не решено |
