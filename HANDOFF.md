...
Обновлено: 2026-08-16. Ветка **`feat/v2-implementation`** (отведена от `feat/tg-mini-app-template`),
синхронна с `origin`.
...
целиком — на обоих пресетах, плюс отдельный докер-прогон.

## С чего начинать следующую сессию

**`V2_PLAN.md §6 «План по сессиям»** — там ритуал начала/конца сессии и разбивка оставшегося.
Сессии A, B и C (шаги 3–5) закрыты. Следующая — **сессия D (шаг 6: админка на `ds/`)**.
Развилка «острова или SPA» уже решена в пользу островов, обоснование и все замеренные ловушки —
в §6, сессия D. Главные из них: `auth.html.erb` не подключает Vite, `tma.rb:33` дописывает
разметку в файл, который сессия D удаляет, а удаления v1-CSS и `_ds_table` **отложены до E**,
потому что ими пока пользуется генератор.
